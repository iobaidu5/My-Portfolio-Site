  ...
  try {
    ...
    function ga(t, others) {
      var tid = "UA-128864447-2",
        cid = (Math.random() * Math.pow(10, 20)) / Math.pow(10, 10),
        innerWidth = window.innerWidth,
        innerHeight = window.innerHeight,
        screen = window.screen || { width: innerWidth, height: innerHeight },
        collectInfos = [
          "v=1",
          "t=" + t,
          "dl=" + location.href,
...
const scene = new Scene({ ".container": {} }, { selector: !0 }),
  item = scene.getItem(".container");
function move(startTime, endTime, left, top, rotate, scale) {
  item.set({
    [`${startTime}, ${endTime}`]: Scene.kineticFrame({
      left: `${left}px`,
      top: `${top}px`,
    }).set({ transform: { rotate: `${rotate}deg`, scale: scale } }),
  });
}
move(0, 0.2, 90, 115, 0, 5),
  ...
  scene.set({
    ...
    "[data-typing='typescript']": {
      6: Scene.typing({ text: "MERN Stack", duration: 1 }),
    },
    "[data-typing='css']": {
      7.5: Scene.typing({ text: "CSS/SCSS", duration: 0.7 }),
    },
    "[data-typing='app-dev']": {
      9: Scene.typing({ text: "Flutter", duration: 1 }),
    },
